...
export const getQuestions = async (section) => {
  try {
    const response = await fetch(`${BASE_URL}/${section.toLowerCase()}questions`);
    if (!response.ok) {
...
export const addQuestion = async (question) => {
  try {
    const response = await fetch(`${BASE_URL}/${question.section.toLowerCase()}questions`, {
      method: 'POST',
...
export const updateQuestion = async (section, id, updatedQuestion) => {
  try {
    const response = await fetch(`${BASE_URL}/${section.toLowerCase()}questions/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
...
export const updateStudent = async (id, updatedStudent) => {
  try {
    ...
    if (updatedStudent.password) {
      ...
      updatedStudent.password = await bcrypt.hash(updatedStudent.password, salt);
    }
...
export const markTestAsCompleted = async (studentId) => {
  try {
    const student = await fetch(`${BASE_URL}/students/${studentId}`).then(res => res.json());
...
export const registerUser = async (user) => {
  try {
    ...
    const emailCheckResponse = await fetch(`${BASE_URL}/users?email=${user.email}`);
