...
import { fetchQuestionsBySection, updateStudent, markTestAsCompleted } from '../api/service';
...
const TestPage = () => {
  ...
  const [score, setScore] = useState({ embedded: { part1: 0, part2: 0, part3: 0 }, vlsi: { part1: 0, part2: 0, part3: 0 } });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isBothTests && section === 'Embedded') {
        ...
        alert(`Test completed! Embedded: ${updatedScore.embedded.part1 + updatedScore.embedded.part2 + updatedScore.embedded.part3}, VLSI: ${updatedScore.vlsi.part1 + updatedScore.vlsi.part2 + updatedScore.vlsi.part3}`);
        navigate('/thank-you');
      }
    } catch (error) {
  ...
  const getQuestionStatus = (questionId) => {
    ...
    } else if (/* You might want to track skipped questions separately */ false) {
      return 'skipped'; // blue
    }
    return '';
  };
    ...
    <div className="test-page">
      ...
      <div className="main-content">
        ...
        <div className="question-container">
          {questionsByPart[currentPart] && questionsByPart[currentPart].length > 0 && (
            <>
              <h3>Questions for {currentPart}</h3>
              {questionsByPart[currentPart].map((q, index) => (
                <div key={q.id} className={`question-box ${answers[q.id] !== undefined ? 'answered' : ''}`}>
                  <p>{index + 1}. {q.question}</p>
                  {q.imageUrl && <img src={q.imageUrl} alt={`Question ${q.id}`} className="question-image" />}
      ...
      <Modal show={showSubmitConfirmation} onHide={() => setShowSubmitConfirmation(false)}>
