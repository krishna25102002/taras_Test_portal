...
const StudentPortal = () => {
      ...
      try {
        ...
        if (enabled.length > 0) {
          setStudentDetails((prevDetails) => ({
            ...prevDetails,
            testSection: enabled[0] === 'both' ? 'Both' : enabled[0].charAt(0).toUpperCase() + enabled[0].slice(1)
          }));
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const { name, class: cls, department, city, collegeName, phoneNumber, testSection } = studentDetails;
    ...
    try {
      ...
      if (testSection === 'Both') {
        navigate('/test/embedded', {
          state: { studentId: addedStudent.id, section: 'Embedded', isBothTests: true }
        });
      } else {
        navigate(`/test/${testSection.toLowerCase()}`, { state: { studentId: addedStudent.id, section: testSection } });
      }
    } catch (error) {
        ...
        <Row className="justify-content-center">
          <Col md={8} lg={6}>
            ...
            {Object.keys(studentDetails)
              .filter((key) => key !== 'name' && key !== 'email' && key !== 'testSection')
              .map((key) => (
            ...
            <Form.Group controlId="formTestSection">
              <Form.Label className="student-portal-label">Test Section</Form.Label>
              <Form.Control
                as="select"
                name="testSection"
                ...
                className="student-portal-input"
              >
                {enabledSections.includes('embedded') && <option value="Embedded">Embedded</option>}
                {enabledSections.includes('vlsi') && <option value="VLSI">VLSI</option>}
                {enabledSections.includes('both') && <option value="Both">Attend Both</option>}
              </Form.Control>
            </Form.Group>

            <Button variant="primary" type="submit" className="student-portal-button">
              Start Test
            </Button>
          </Col>
        </Row>
